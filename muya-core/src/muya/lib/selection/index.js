...
class Selection {
  ...
  importSelection(selectionState, root, favorLaterSelectionAnchor) {
    ...
    while (!stop && node) {
      ...
      if (node.nodeType === 3 && !foundEnd) {
        ...
        if (
          node.nodeType === 1 &&
          (node.classList.contains(CLASS_OR_ID.AG_MATH_RENDER) ||
            node.classList.contains(CLASS_OR_ID.AG_RUBY_RENDER))
        ) {
          node = nodeStack.pop()
          continue
        }
        ...
        if (node.nodeType === 1 && node.classList.contains('ag-inline-image') && !node.classList.contains('ag-empty-image')) {
          ...
          if (
            ...
            range.setStart(parent, selectionState.start - charIndex > 0 ? index + 1 : index)
            foundStart = true
          }
          ...
          if (foundStart && selectionState.end >= charIndex && selectionState.end <= nextCharIndex) {
            const parent = node.parentNode
            const index = Array.from(parent.childNodes).indexOf(node)
            range.setEnd(parent, selectionState.end - charIndex > 0 ? index + 1 : index)
            stop = true
          }
  ...
  getCursorRange() {
    ...
    const calculateOffset = (node, paragraph, offset) => {
      let result = getOffsetOfParagraph(node, paragraph)
      if (node.nodeType === 3) {
        result += offset
      } else {
    ...
    let aOffset = calculateOffset(anchorNode, anchorParagraph, anchorOffset)
    let fOffset = calculateOffset(focusNode, focusParagraph, focusOffset)
