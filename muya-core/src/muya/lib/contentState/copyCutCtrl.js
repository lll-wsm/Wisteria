...
const copyCutCtrl = (ContentState) => {
  ...
  ContentState.prototype.getClipBoardData = function() {
    ...
    let html = selection.getSelectionHtml()
    if (!html) {
      const startBlock = this.getBlock(start.key)
      const endBlock = this.getBlock(end.key)
      if (startBlock && endBlock) {
        ...
        while (block && block !== endBlock) {
          ...
          block = this.findNextBlockInLocation(block)
        }
        selectedBlocks.push(endBlock)
        
        const lines = selectedBlocks.map(b => {
          if (b.key === start.key && b.key === end.key) {
            return b.text.substring(start.offset, end.offset)
          }
          if (b.key === start.key) {
            return b.text.substring(start.offset)
          }
          if (b.key === end.key) {
            return b.text.substring(0, end.offset)
          }
          return b.text
        })
        const textData = lines.join('\n')
        return { html: marked(textData, this.muya.options), text: textData }
      }
      return { html: '', text: '' }
    }
