...
const tabCtrl = (ContentState) => {
  ...
  ContentState.prototype.indentListItemByBlock = function(listItem) {
    const list = this.getParent(listItem)
    if (!list || !/ol|ul/.test(list.type)) return false
    const prevListItem = this.getPreSibling(listItem)
    if (!prevListItem) return false // Cannot indent if there is no previous sibling
  ...
  ContentState.prototype.unindentListItemByBlock = function(listItem) {
    const list = this.getParent(listItem)
    if (!list || !/ol|ul/.test(list.type)) return false
    const listParent = this.getParent(list)
    if (!listParent || listParent.type !== 'li') return false // Top level lists cannot be unindented further in current design

    const type = !list.preSibling ? 'REPLACEMENT' : 'INDENT'
    
    // Find the text block inside listItem to pass to unindentListItem
    const firstTextSpan = this.firstInDescendant(listItem)
    if (firstTextSpan) {
      this.unindentListItem(firstTextSpan, type)
      return true
    }
    return false
  }

  ContentState.prototype.multilineTabHandler = function(event, start, end, startBlock, endBlock) {
    const isShift = event.shiftKey
    const tabSize = this.tabSize || 4
    const tabCharacter = ' '.repeat(tabSize)

    // 1. Gather all leaf blocks in selection
    const selectedBlocks = []
    let currentBlock = startBlock
    while (currentBlock && currentBlock !== endBlock) {
      selectedBlocks.push(currentBlock)
      currentBlock = this.findNextBlockInLocation(currentBlock)
    }
    selectedBlocks.push(endBlock)

    // Helper: check if a block belongs to a list item (li)
    const getListItem = (block) => {
      let parent = this.getParent(block)
      while (parent) {
        if (parent.type === 'li') return parent
        parent = this.getParent(parent)
      }
      return null
    }

    // 2. Identify list items and text blocks
    const listItems = []
    const seenListItems = new Set()
    const textBlocks = []

    for (const b of selectedBlocks) {
      const li = getListItem(b)
      if (li) {
        if (!seenListItems.has(li.key)) {
          seenListItems.add(li.key)
          listItems.push(li)
        }
      } else {
        // Only include editable text/content blocks
        if (b.type === 'span' && /atxLine|paragraphContent|cellContent|codeContent/.test(b.functionType || b.type)) {
          textBlocks.push(b)
        }
      }
    }

    // 3. Process List Items
    if (listItems.length > 0) {
      if (isShift) {
        // Unindent list items (Shift+Tab)
        for (const li of listItems) {
          this.unindentListItemByBlock(li)
        }
      } else {
        // Indent list items (Tab)
        for (const li of listItems) {
          this.indentListItemByBlock(li)
        }
      }
    }

    // 4. Process Normal Text Blocks
    let startStripped = 0
    let endStripped = 0
    ...
    if (textBlocks.length > 0) {
      if (isShift) {
        // Outdent text
        const stripIndent = (text, size) => {
          if (text.startsWith('\t')) {
            return { text: text.substring(1), stripped: 1 }
          }
          let spaceCount = 0
          while (spaceCount < size && text.charAt(spaceCount) === ' ') {
            spaceCount++
          }
          if (spaceCount > 0) {
            return { text: text.substring(spaceCount), stripped: spaceCount }
          }
          return { text, stripped: 0 }
        }

        for (const b of textBlocks) {
          const { text: newText, stripped } = stripIndent(b.text, tabSize)
          b.text = newText
          if (b.key === start.key) {
            startStripped = stripped
          }
          if (b.key === end.key) {
            endStripped = stripped
          }
        }

        this.cursor = {
          start: { key: start.key, offset: Math.max(0, start.offset - startStripped) },
          end: { key: end.key, offset: Math.max(0, end.offset - endStripped) },
          isEdit: false
        }
      } else {
        // Indent text
        for (const b of textBlocks) {
          b.text = tabCharacter + b.text
        }

        this.cursor = {
          start: { key: start.key, offset: start.offset + tabCharacter.length },
          end: { key: end.key, offset: end.offset + tabCharacter.length },
          isEdit: false
        }
      }
    } else {
      // If we only processed list items, keep the cursor selection
      this.cursor = { start, end, isEdit: false }
    }
