...
class ContentState {
  ...
  setNextRenderRange() {
    ...
    const { start: pStart, end: pEnd } = this.prevCursor || {}
    const startBlock = this.getBlock(start ? start.key : null)
    const endBlock = this.getBlock(end ? end.key : null)
    const pStartBlock = this.getBlock(pStart ? pStart.key : null)
    const pEndBlock = this.getBlock(pEnd ? pEnd.key : null)
    ...
    const pStartOutMostBlock = pStartBlock ? this.findOutMostBlock(pStartBlock) : null
    const pEndOutMostBlock = pEndBlock ? this.findOutMostBlock(pEndBlock) : null

    const blocks = [startOutMostBlock, endOutMostBlock, pStartOutMostBlock, pEndOutMostBlock].filter(b => !!b)
    const indices = blocks.map(b => this.blocks.indexOf(b))
    const minIndex = Math.min(...indices)
    const maxIndex = Math.max(...indices)

    this.renderRange = [
      this.blocks[minIndex].preSibling,
      this.blocks[maxIndex].nextSibling
    ]
  }
