...
class StateRender {
  ...
  getSelector(block, activeBlocks) {
    ...
    const hasFocus = this.muya.hasFocus()
    const isActive = hasFocus && (activeBlocks.some((b) => b.key === block.key) || block.key === cursor.start.key)
  ...
  async renderMermaid() {
    if (this.mermaidCache.size) {
      ...
      if (!this.mermaidRenderVersion) {
        this.mermaidRenderVersion = new Map()
      }
      ...
      for (const [key, value] of this.mermaidCache.entries()) {
        ...
        try {
          ...
          const id = `mermaid-${key.slice(1)}-${currentVersion}`
  ...
  async renderDiagram() {
    ...
    if (cache.size) {
      ...
      if (!this.diagramRenderVersion) {
        this.diagramRenderVersion = new Map()
      }
      ...
      for (const [key, value] of cache.entries()) {
        ...
        const currentVersion = (this.diagramRenderVersion.get(key) || 0) + 1
        this.diagramRenderVersion.set(key, currentVersion)

        try {
          if (functionType === 'flowchart' || functionType === 'sequence') {
            const diagram = render.parse(code)
            if (this.diagramRenderVersion.get(key) !== currentVersion) {
              continue
            }
            target.innerHTML = ''
