...
- **桌面级功能**：
  ...
  - 高质量 PDF 导出（采用前端渲染库 `html2pdf.js`，保证样式高度一致）。
...
### 开发模式
Wisteria 使用 Vite 驱动前端，Tauri v2 驱动桌面后端。
```bash
# 启动开发服务器（包含前端 Vite 与 Tauri 后端）
npm run dev
```
...
- `src/`: Wisteria 前端应用源码。
  - `renderer.js`: 前端逻辑与 Tauri IPC 交互。
  - `style.css`: 经过优化的全局样式。
- `src-tauri/`: Tauri 后端（Rust）源码。
  - `src/main.rs`: 应用程序入口。
  - `src/lib.rs`: 核心后端逻辑、原生菜单与 IPC 命令。
